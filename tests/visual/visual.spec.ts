...
const only = process.env.VISUAL_PAGES?.split(',').map(s => s.trim()).filter(Boolean);

for (const p of pages) {
  if (only && !only.includes(p.name)) continue;
  const variants: { width: number; dark: boolean }[] = [{ width: 1540, dark: false }];
  ...
  for (const v of variants) {
    ...
    test(label, async ({ browser }) => {
      ...
      test.info().annotations.push({ type: 'diff', description: `${(r.ratio * 100).toFixed(2)}% (${r.diffPixels}px), height Δ ${r.heightDelta}px` });
      await test.info().attach('diff', { path: r.files.diff, contentType: 'image/png' });
      await test.info().attach('react', { path: r.files.react, contentType: 'image/png' });
      await test.info().attach('gold', { path: r.files.gold, contentType: 'image/png' });
      expect(
        r.ratio,
