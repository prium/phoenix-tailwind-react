...
6. **Leftover grep must be empty** for the touched dirs (one known false positive: Tailwind v4's own `bg-position-[…]` / `bg-size-[…]` arbitrary utilities, which the gold uses on `.bg-holder` — keep them):
...
- Chart tokens (`getThemeColor`): `primary→color-primary`, `primary-bg-subtle→color-primary-subtle`, `body-bg→background-color-default`, `body-color→text-color-default`, `secondary-color→text-color-muted`, `tertiary-bg→background-color-highlight`, `quaternary-color→text-color-soft`, `border-color→border-color-default`, `gray-N→color-gray-N`. `getColor(name)` reads `--${name}`; a dev warning means the token name is wrong. Dead tokens resolve to '' and echarts paints BLACK/grey — and **canvas is masked in the visual suite**, so after porting any page with charts, view an UNMASKED screenshot of both sides (`tools/verify/shot.mjs`) and compare chart colors by eye. Renamed upstream: `border-color-base→border-color-default`, `border-color-light→border-color-subtle`. Short color names must be prefixed (`color-warning`, not `warning`); heatmap/calendar cells use `color-contrast`.
