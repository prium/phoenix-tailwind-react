...
3. Component policy: Hummingbird React parts wherever their DOM matches the gold; where it diverges or the gold uses plugin markup, use the existing app wrappers (skill §Component policy lists them) — and if none fits, CREATE a named component under `components/base|common` with a stable prop API rather than inlining raw gold HTML across pages. Put literal classes in data files when they are dynamic.
